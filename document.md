...
*Bookstore*是一个**书店管理系统**，面向书店的管理者们和受众们。对书店的顾客而言，该程序提供了方便的图书检索与购买相关服务；对书店管理者和员工而言，它提供了便捷的用户系统和方便的进货、检索、用户管理等功能。
...
同时，我实现了"explain.h"这一个头文件，在其中我们经过切片来确定需要执行"program.h"中的哪一种语句中的哪一个细化类别（例如无参数的show和有参数的show），并分析其语法有效性，如果不有效则抛出异常，输出"invalid\n"。

剩下的将交给"program.h"，其中有针对不同语句的执行方案，并涵盖了用户、书本等文件，直接与文件交互，并在控制台输出需要的结果。
